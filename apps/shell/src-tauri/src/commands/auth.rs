...
use clubshell_protocol::commands::{names, AuthLoginRequest, AuthLoginResponse, AuthLogoutRequest, AuthLogoutResponse, AuthStatusResponse};
...
#[tauri::command]
pub async fn auth_login(state: State<'_, AppState>, req: AuthLoginRequest) -> CmdResult<AuthLoginResponse> {
...
#[tauri::command]
pub async fn auth_logout(state: State<'_, AppState>, reason: Option<SessionEndReason>) -> CmdResult<AuthLogoutResponse> {
    if let Some(reason) = reason {
        if !matches!(reason, SessionEndReason::User | SessionEndReason::Idle | SessionEndReason::Admin) {
            return Err(ShellError::validation("reason", "must be user, idle or admin"));
        }
    }
    let resp: AuthLogoutResponse = state.agent.request(names::auth::LOGOUT, &AuthLogoutRequest { reason }).await?;
    state.set_user(None);
...
fn validate_login(mut req: AuthLoginRequest) -> CmdResult<AuthLoginRequest> {
    match req.kind {
        AuthKind::Password => {
            req.username = Some(validate::required_text("username", req.username.as_deref().unwrap_or_default(), USERNAME_MAX)?);
        ...
        AuthKind::Qr => {
            req.qr_token = Some(validate::required_text("qrToken", req.qr_token.as_deref().unwrap_or_default(), TOKEN_MAX)?);
        }
        AuthKind::Card => {
            req.card_id = Some(validate::required_text("cardId", req.card_id.as_deref().unwrap_or_default(), TOKEN_MAX)?);
        }
        AuthKind::Token => {
            req.token = Some(validate::required_text("token", req.token.as_deref().unwrap_or_default(), TOKEN_MAX)?);
        }
        AuthKind::Guest => {}
    }
    Ok(req)
...
mod tests {
    ...
    fn login(kind: AuthKind) -> AuthLoginRequest {
        AuthLoginRequest { kind, username: None, password: None, qr_token: None, card_id: None, token: None }
    }

    #[test]
    fn password_login_requires_username_and_password() {
        let e = validate_login(AuthLoginRequest { username: Some("bob".into()), ..login(AuthKind::Password) }).unwrap_err();
        assert_eq!(e.code, ErrorCode::Validation);
        assert_eq!(e.details.unwrap()["field"], "password");
        let e = validate_login(AuthLoginRequest { password: Some("x".into()), ..login(AuthKind::Password) }).unwrap_err();
        assert_eq!(e.details.unwrap()["field"], "username");
        let ok = validate_login(AuthLoginRequest { username: Some(" bob ".into()), password: Some(" x ".into()), ..login(AuthKind::Password) }).unwrap();
        assert_eq!(ok.username.as_deref(), Some("bob"));
        assert_eq!(ok.password.as_deref(), Some(" x "), "passwords are never trimmed");
    }

    #[test]
    fn other_kinds_require_their_secret() {
        assert!(validate_login(login(AuthKind::Qr)).is_err());
        assert!(validate_login(AuthLoginRequest { qr_token: Some("t".into()), ..login(AuthKind::Qr) }).is_ok());
