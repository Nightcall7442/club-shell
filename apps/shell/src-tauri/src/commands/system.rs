...
use clubshell_protocol::commands::{
    names, CallAdminCategory, ClientErrorLevel, OkResponse, PolicyReloadRequest, PolicyReloadResponse, ScheduledResult,
    SetVolumeRequest, SysAckAdminMessageRequest, SysCallAdminRequest, SysCallAdminResponse, SysHardwareRequest,
    SysLockScreenRequest, SysLogClientErrorRequest, SysPowerRequest, SysSetLocaleRequest, SysSetLocaleResponse,
    SysUnlockAdminRequest, SysUnlockAdminResponse, UpdateApplyRequest, UpdateApplyResponse, UpdateCheckResponse,
    UpdateComponent, VolumeState,
};
...
#[tauri::command]
pub async fn sys_hardware(state: State<'_, AppState>, refresh: Option<bool>) -> CmdResult<HardwareInfo> {
    state.agent.request(names::sys::HARDWARE, &SysHardwareRequest { refresh }).await
}
...
#[tauri::command]
pub async fn sys_call_admin(state: State<'_, AppState>, category: CallAdminCategory, message: Option<String>) -> CmdResult<SysCallAdminResponse> {
    let message = validate::optional_text("message", message, CALL_ADMIN_MESSAGE_MAX)?;
    let resp: SysCallAdminResponse = state.agent.request(names::sys::CALL_ADMIN, &SysCallAdminRequest { category, message }).await?;
...
#[tauri::command]
pub async fn sys_reboot(state: State<'_, AppState>, delay_sec: Option<i32>, reason: Option<String>) -> CmdResult<ScheduledResult> {
...
#[tauri::command]
pub async fn sys_shutdown(state: State<'_, AppState>, delay_sec: Option<i32>, reason: Option<String>) -> CmdResult<ScheduledResult> {
    let req = validate_power(delay_sec, reason)?;
...
#[tauri::command]
pub async fn sys_lock_screen(state: State<'_, AppState>, reason: Option<String>) -> CmdResult<OkResponse> {
    let reason = validate::optional_text("reason", reason, REASON_MAX)?;
    state.agent.request(names::sys::LOCK_SCREEN, &SysLockScreenRequest { reason }).await
}

/// `sys_set_volume` → `sys.setVolume`; `level` 0–100, `muted` unchanged when absent.
#[tauri::command]
pub async fn sys_set_volume(state: State<'_, AppState>, level: i32, muted: Option<bool>) -> CmdResult<VolumeState> {
    validate::range("level", level, 0, 100)?;
    state.agent.request(names::sys::SET_VOLUME, &SetVolumeRequest { level, muted }).await
}

/// `sys_set_locale` → `sys.setLocale`, then `kiosk://localeChanged`.
#[tauri::command]
pub async fn sys_set_locale(app: AppHandle, state: State<'_, AppState>, locale: Locale) -> CmdResult<SysSetLocaleResponse> {
    let resp: SysSetLocaleResponse = state.agent.request(names::sys::SET_LOCALE, &SysSetLocaleRequest { locale }).await?;
...
#[tauri::command]
pub async fn sys_unlock_admin(state: State<'_, AppState>, pin: String) -> CmdResult<SysUnlockAdminResponse> {
    let pin = pin.trim().to_owned();
    if pin.is_empty() {
        return Err(ShellError::validation("pin", "required"));
    }
    if pin.chars().count() > ADMIN_PIN_MAX {
        return Err(ShellError::validation("pin", &format!("must be at most {ADMIN_PIN_MAX} characters")));
    }
    let resp: SysUnlockAdminResponse = state.agent.request(names::sys::UNLOCK_ADMIN, &SysUnlockAdminRequest { pin }).await?;
...
pub async fn sys_ack_admin_message(state: State<'_, AppState>, id: Uuid) -> CmdResult<OkResponse> {
    ...
    state.agent.request(names::sys::ACK_ADMIN_MESSAGE, &SysAckAdminMessageRequest { id }).await
}
...
#[tauri::command]
pub async fn sys_log_client_error(state: State<'_, AppState>, e: SysLogClientErrorRequest) -> CmdResult<()> {
    let req = SysLogClientErrorRequest {
        level: e.level,
        message: truncate(e.message.trim(), CLIENT_ERROR_MESSAGE_MAX),
        stack: e.stack.as_deref().map(str::trim).filter(|s| !s.is_empty()).map(|s| truncate(s, CLIENT_ERROR_STACK_MAX)),
        route: e.route.as_deref().map(str::trim).filter(|s| !s.is_empty()).map(|s| truncate(s, CLIENT_ERROR_ROUTE_MAX)),
    };
    match req.level {
        ClientErrorLevel::Error => tracing::error!(target: "webview", route = ?req.route, stack = ?req.stack, "{}", req.message),
        ClientErrorLevel::Warn => tracing::warn!(target: "webview", route = ?req.route, stack = ?req.stack, "{}", req.message),
    }
    let agent = Arc::clone(&state.agent);
    tauri::async_runtime::spawn(async move {
        let payload = serde_json::to_value(&req).ok();
        if let Err(err) = agent.request_raw(names::sys::LOG_CLIENT_ERROR, payload).await {
...
#[tauri::command]
pub async fn policy_reload(state: State<'_, AppState>, force: Option<bool>) -> CmdResult<PolicyReloadResponse> {
    let resp: PolicyReloadResponse = state.agent.request(names::policy::RELOAD, &PolicyReloadRequest { force }).await?;
...
#[tauri::command]
pub async fn update_apply(state: State<'_, AppState>, component: UpdateComponent) -> CmdResult<UpdateApplyResponse> {
    let resp: UpdateApplyResponse = state.agent.request(names::update::APPLY, &UpdateApplyRequest { component }).await?;
...
mod tests {
    ...
    fn power_request_is_checked() {
        ...
        assert_eq!((ok.delay_sec, ok.reason.as_deref()), (Some(30), Some("admin")));
        assert_eq!(validate_power(None, None).unwrap(), SysPowerRequest::default());
    }
}
