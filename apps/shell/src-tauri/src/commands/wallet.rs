...
use clubshell_protocol::commands::{names, WalletHistoryRequest, WalletHistoryResponse, WalletTariffsRequest, WalletTariffsResponse, WalletTopupIntentRequest};
use clubshell_protocol::wallet::{Balance, Money, TopupIntent, TopupIntentCreateRequest, TopupProvider};
use tauri::State;
...
#[tauri::command]
pub async fn wallet_tariffs(state: State<'_, AppState>, zone: Option<String>) -> CmdResult<WalletTariffsResponse> {
    let zone = validate::optional_text("zone", zone, ZONE_MAX)?;
    state.agent.request(names::wallet::TARIFFS, &WalletTariffsRequest { zone }).await
}

/// `wallet_history` → `wallet.history`.
#[tauri::command]
pub async fn wallet_history(state: State<'_, AppState>, q: Option<WalletHistoryRequest>) -> CmdResult<WalletHistoryResponse> {
...
#[tauri::command]
pub async fn wallet_topup_intent(state: State<'_, AppState>, amount: Money, provider: TopupProvider) -> CmdResult<TopupIntent> {
    if amount.amount < TopupIntentCreateRequest::MIN_AMOUNT_MINOR {
        return Err(ShellError::validation(
            "amount",
            &format!("must be at least {} minor units", TopupIntentCreateRequest::MIN_AMOUNT_MINOR),
        ));
    }
    let intent: TopupIntent = state.agent.request(names::wallet::TOPUP_INTENT, &WalletTopupIntentRequest { amount, provider }).await?;
