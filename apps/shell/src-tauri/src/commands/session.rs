...
use clubshell_protocol::commands::{
    names, BookingCancelRequest, BookingReserveRequest, BookingSeatsRequest, BookingSeatsResponse,
    ProfileAchievementsResponse, SessionEndRequest, SessionExtendRequest, SessionLockRequest,
    SessionPauseRequest, SessionStartRequest, SessionTimeLeftResponse, SessionUnlockRequest,
...
async fn call_session<Req: Serialize + ?Sized>(
    state: &AppState,
    name: &str,
    req: &Req,
) -> CmdResult<Session> {
...
async fn call_user<Req: Serialize + ?Sized>(
    state: &AppState,
    name: &str,
    req: &Req,
) -> CmdResult<User> {
...
) -> CmdResult<SessionEndResult> {
    let result: SessionEndResult = state
        .agent
        .request(names::session::END, &SessionEndRequest { reason })
        .await?;
...
pub async fn session_extend(
    ...
    call_session(
        &state,
        names::session::EXTEND,
        &SessionExtendRequest { minutes, tariff_id },
    )
    .await
}
...
fn validate_unlock(
    password: Option<String>,
    pin: Option<String>,
) -> CmdResult<SessionUnlockRequest> {
    ...
    match (password, pin) {
        ...
        (Some(_), Some(_)) => Err(ShellError::validation(
            "pin",
            "give either password or pin, not both",
        )),
        (Some(password), None) => {
...
#[tauri::command]
pub async fn booking_seats(
    state: State<'_, AppState>,
    date: NaiveDate,
) -> CmdResult<BookingSeatsResponse> {
    state
        .agent
        .request(names::booking::SEATS, &BookingSeatsRequest { date })
        .await
}

/// `booking_reserve` → `booking.reserve`; `from < to`, at most 24 h, not in the past.
#[tauri::command]
pub async fn booking_reserve(
    state: State<'_, AppState>,
    pc_id: Uuid,
    from: DateTime<Utc>,
    to: DateTime<Utc>,
    ...
    state
        .agent
        .request(
            names::booking::RESERVE,
            &BookingReserveRequest { pc_id, from, to },
        )
        .await
}
...
pub async fn booking_cancel(state: State<'_, AppState>, booking_id: Uuid) -> CmdResult<Booking> {
    state
        .agent
        .request(names::booking::CANCEL, &BookingCancelRequest { booking_id })
        .await
}
...
fn validate_booking(
    ...
    if to - from > Duration::hours(BOOKING_MAX_HOURS) {
        return Err(ShellError::validation(
            "to",
            &format!("booking longer than {BOOKING_MAX_HOURS} hours"),
        ));
    }
    if to <= now {
...
#[tauri::command]
pub async fn tournaments_list(
    state: State<'_, AppState>,
    q: Option<TournamentsListRequest>,
) -> CmdResult<Vec<Tournament>> {
    let resp: TournamentsListResponse = state
        .agent
        .request(names::tournaments::LIST, &q.unwrap_or_default())
        .await?;
    Ok(resp.items)
}
...
#[tauri::command]
pub async fn tournaments_join(
    state: State<'_, AppState>,
    tournament_id: Uuid,
) -> CmdResult<Tournament> {
    state
        .agent
        .request(
            names::tournaments::JOIN,
            &TournamentsJoinRequest { tournament_id },
        )
        .await
}
...
pub async fn tournaments_leaderboard(
    ...
    state
        .agent
        .request(
            names::tournaments::LEADERBOARD,
            &TournamentsLeaderboardRequest {
                tournament_id,
                limit,
            },
        )
...
pub async fn profile_achievements(state: State<'_, AppState>) -> CmdResult<Vec<Achievement>> {
    let resp: ProfileAchievementsResponse = state
        .agent
        .request(names::profile::ACHIEVEMENTS, &())
        .await?;
    Ok(resp.items)
}
...
fn validate_profile_patch(mut patch: ProfileUpdateRequest) -> CmdResult<ProfileUpdateRequest> {
    ...
    if let Some(name) = patch.display_name.take() {
        patch.display_name = Some(validate::required_text(
            "displayName",
            &name,
            DISPLAY_NAME_MAX,
        )?);
...
mod tests {
    ...
    fn unlock_needs_exactly_one_secret() {
        ...
        assert_eq!(
            validate_unlock(None, Some(" 1234 ".into()))
                .unwrap()
                .pin
                .as_deref(),
            Some("1234")
        ...
        assert_eq!(
            validate_unlock(Some("pw".into()), None)
                .unwrap()
                .password
                .as_deref(),
            Some("pw")
        ...
        assert!(
            validate_unlock(Some(String::new()), None).is_err(),
    ...
    fn booking_window_is_checked() {
        ...
        assert!(
            validate_booking(pc, now + Duration::hours(1), now + Duration::hours(2), now).is_ok()
        );
        ...
        .is_err());
        assert!(
            validate_booking(pc, now + Duration::hours(2), now + Duration::hours(1), now).is_err()
        );
        assert!(
            validate_booking(pc, now + Duration::hours(1), now + Duration::hours(26), now).is_err()
        );
        assert!(
            validate_booking(pc, now - Duration::hours(3), now - Duration::hours(1), now).is_err()
        );
    }
    ...
    fn profile_patch_is_checked() {
        ...
        let ok = validate_profile_patch(ProfileUpdateRequest {
            display_name: Some(" Bob ".into()),
            ..Default::default()
        })
        .unwrap();
        assert_eq!(ok.display_name.as_deref(), Some("Bob"));
        assert!(validate_profile_patch(ProfileUpdateRequest {
            avatar_url: Some("ftp://x".into()),
            ..Default::default()
        })
        .is_err());
        assert!(validate_profile_patch(ProfileUpdateRequest {
