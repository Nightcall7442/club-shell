/**
 * UI sounds synthesised with WebAudio (no assets): a focus tick, a press and a confirmation chime. Gated by
 * `settings.uiSounds`, `muted` and `volume`. The context is created lazily on the first call, which always follows
 * a user gesture (focus, pointer), so autoplay policy never blocks it.
 */
