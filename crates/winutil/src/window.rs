...
pub fn alt_tab_combos() -> Vec<BlockedCombo> {
    ["Alt+Tab", "Alt+Esc", "Win+Tab", "Ctrl+Alt+Tab"]
        .iter()
        .map(|s| BlockedCombo::parse(s).expect("built-in combos parse"))
        .collect()
}
...
mod imp {
    ...
    use windows::Win32::UI::WindowsAndMessaging::{
        ...
        SetForegroundWindow, SetWindowDisplayAffinity, SetWindowPos, ShowWindow, GWL_EXSTYLE,
        GWL_STYLE, HWND_NOTOPMOST, HWND_TOPMOST, SWP_FRAMECHANGED, SWP_NOACTIVATE, SWP_NOMOVE,
        SWP_NOOWNERZORDER, SWP_NOSIZE, SWP_NOZORDER, SWP_SHOWWINDOW, SW_FORCEMINIMIZE, SW_HIDE,
        SW_SHOW, WDA_EXCLUDEFROMCAPTURE, WDA_NONE, WINDOW_LONG_PTR_INDEX, WS_CAPTION,
    ...
    use crate::hooks::vk;
    use crate::{hwnd_from_raw, last_error, Win32Ret, WinUtilError};
    ...
    pub fn find_window(class: Option<&str>, title: Option<&str>) -> Result<Option<isize>> {
        if class.is_none() && title.is_none() {
            return Err(WinUtilError::Invalid(
                "find_window needs a class or a title".to_owned(),
            ));
        }
        Ok(enumerate_windows()?
            .into_iter()
            .find(|w| {
                let class_ok = if let Some(c) = class {
                    w.class.eq_ignore_ascii_case(c)
                } else {
                    true
                };
                let title_ok = if let Some(t) = title {
                    w.title == t
                } else {
                    true
                };
    ...
    pub fn window_pid(hwnd: isize) -> Result<u32> {
        ...
        let tid =
            unsafe { GetWindowThreadProcessId(hwnd_from_raw(hwnd), Some(&mut pid as *mut u32)) };
        if tid == 0 {
    ...
    pub fn force_foreground(hwnd: isize) -> Result<bool> {
        ...
        unsafe {
            ...
            let attached = fg_thread != 0
                && fg_thread != me
                && AttachThreadInput(me, fg_thread, BOOL::from(true)).as_bool();
    ...
    pub fn taskbar_autohide(enable: bool) -> Result<()> {
        let tray = find_window(Some("Shell_TrayWnd"), None)?
            .ok_or_else(|| WinUtilError::Invalid("taskbar window not found".to_owned()))?;
        let state: u32 = if enable {
            ABS_AUTOHIDE
        } else {
            ABS_ALWAYSONTOP
        };
    ...
    pub fn set_window_display_affinity(hwnd: isize, exclude_from_capture: bool) -> Result<()> {
        ...
        // SAFETY: no preconditions beyond a window handle.
        unsafe { SetWindowDisplayAffinity(hwnd_from_raw(hwnd), affinity) }
            .ret("SetWindowDisplayAffinity")
    }
...
pub use imp::{
    child_windows, enumerate_windows, find_window, force_foreground, foreground_window,
    hide_taskbar, is_foreground, minimize_others, set_fullscreen_borderless, set_topmost,
    set_window_display_affinity, show_taskbar, taskbar_autohide, window_pid, window_rect,
    TopmostGuard,
};
...
mod tests {
    ...
    fn alt_tab_combos_parse_and_match() {
        ...
        let alt_tab = crate::hooks::KeyEvent {
            vk: crate::hooks::vk::TAB,
            scan: 0,
            flags: 0,
            alt: true,
            ctrl: false,
