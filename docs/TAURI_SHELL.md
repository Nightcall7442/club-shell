...
| `overlay.rs` | `Overlay` (`OVERLAY_LABEL = "overlay"`, url `index.html#/overlay`) | Lazily created transparent always-on-top webview spanning the virtual screen. Kinds: `lock`, `ads`, `message`, `hud`, `none`. Broadcasts `kiosk://overlay { kind, payload }`; `lock` and `hud` take input, the rest are click-through. `hud` is toggled natively by the `Ctrl+Shift+H` hotkey (`toggle_hud`, 60 s TTL backstop) and rendered by `screens/Overlay/HudScreen.tsx`. |
