...
async fn connect_is_unsupported_off_windows() {
    ...
    let err = PipeClient::connect("clubshell-test", std::time::Duration::from_millis(10))
        .await
        .err()
...
mod windows_pipe {
    ...
    use clubshell_protocol::commands::{
        names, AuthHelloRequest, AuthHelloResponse, SysPingRequest,
    };
        ...
        match req.name.as_str() {
            ...
            names::sys::PING => {
                ...
                let sent_at = req
                    .payload
                    .as_ref()
                    .and_then(|p| p.get("sentAt").cloned())
                    .unwrap_or(Value::Null);
                Some(IpcEnvelope::reply_to(
                    req,
                    Some(
                        json!({ "seq": ping.seq, "sentAt": sent_at, "receivedAt": sent_at, "connectivity": "online" }),
                    ),
                ))
            }
    ...
    async fn connect(server: &TestServer, options: PipeOptions) -> anyhow::Result<PipeClient> {
        PipeClient::connect_with(server.name(), CONNECT_TIMEOUT, options)
            .await
            .context("connect")
    }
    ...
    async fn hello(client: &PipeClient) -> anyhow::Result<AuthHelloResponse> {
        client
            .call::<_, AuthHelloResponse>(names::auth::HELLO, &hello_request())
            .await
            .context("auth.hello")
    }
    ...
    async fn connect_hello_and_heartbeat_over_a_real_pipe() -> anyhow::Result<()> {
        ...
        let server = TestServer::start(Arc::new(
            move |req: IpcEnvelope, out: mpsc::Sender<IpcEnvelope>| {
        ...
        assert!(
            ...
            "expected several pings at a 60 ms interval"
        );

        client.close();
        assert_eq!(
            tokio::time::timeout(WAIT, client.closed()).await?,
            CloseReason::Shutdown
        );
        assert_eq!(
            *client.state().borrow(),
            ConnectionState::Closed(CloseReason::Shutdown)
        );
        assert!(matches!(
            client
    ...
    async fn concurrent_requests_are_correlated_by_id() -> anyhow::Result<()> {
        ...
        let server = TestServer::start(Arc::new(
            |req: IpcEnvelope, out: mpsc::Sender<IpcEnvelope>| {
                let n = req
                    .payload
                    .as_ref()
                    .and_then(|p| p.get("n"))
    ...
    async fn events_reach_every_subscriber() -> anyhow::Result<()> {
        ...
        let event = wire(IpcEnvelope::event(
            names::events::SESSION_UPDATED,
            Some(json!({ "state": "active", "secondsLeft": 120 })),
        ));
        ...
        client
            .request(IpcEnvelope::request(names::auth::STATUS, None))
            .await?;
        let second = wire(IpcEnvelope::event(
    ...
    async fn request_times_out_when_the_server_withholds_the_response() -> anyhow::Result<()> {
        ...
        let err = client
            .request(IpcEnvelope::request(names::games::LIST, None))
            .await
            .unwrap_err();
        assert!(matches!(err, WinUtilError::Timeout), "{err}");
        let elapsed = started.elapsed();
        assert!(
            elapsed >= Duration::from_millis(180) && elapsed < Duration::from_secs(2),
            "elapsed {elapsed:?}"
        );
        ...
        let err = client
            .request_timeout(
                IpcEnvelope::request(names::games::LIST, None),
    ...
    async fn server_drop_closes_the_connection_and_fails_pending_requests() -> anyhow::Result<()> {
        ...
        let pending = {
            let c = Arc::clone(&client);
            tokio::spawn(async move {
                c.request(IpcEnvelope::request(names::games::LIST, None))
                    .await
            })
        };
        ...
        assert!(
            matches!(reason, CloseReason::PeerClosed | CloseReason::Io(_)),
            "{reason:?}"
        );
        assert!(!client.is_connected());
        ...
        assert_eq!(
            *state.borrow_and_update(),
            ConnectionState::Closed(reason.clone())
        );
        ...
        client.close();
        assert!(
            matches!(&*client.state().borrow(), ConnectionState::Closed(r) if *r != CloseReason::Shutdown)
        );
        Ok(())
    }
    ...
    async fn busy_pipe_is_retried_until_an_instance_frees_up() -> anyhow::Result<()> {
        ...
        let err = PipeClient::connect_with(
            server.name(),
        ...
        assert!(
            tokio::time::timeout(Duration::from_millis(300), &mut second)
                .await
                .is_err(),
            "must still be waiting while busy"
        );
        assert_eq!(server.accepted(), 1);
    ...
    async fn connect_waits_for_the_server_to_appear() -> anyhow::Result<()> {
        ...
        let started = tokio::time::Instant::now();
        let err =
            PipeClient::connect_with(&missing, Duration::from_millis(120), opts(1_000, false))
                .await
                .err()
                .expect("no server");
        assert!(matches!(err, WinUtilError::Timeout), "{err}");
    ...
    async fn oversized_frames_are_rejected() -> anyhow::Result<()> {
        ...
        let huge = IpcEnvelope::request(
            names::chat::SEND,
            Some(json!({ "text": "a".repeat(MAX_FRAME_BYTES) })),
        );
        let err = client.request(huge).await.unwrap_err();
        assert!(
            matches!(
                err,
                WinUtilError::Frame(clubshell_protocol::error::ProtocolError::FrameTooLarge { .. })
            ),
            "{err}"
        );
        assert!(client.is_connected());
    ...
    async fn typed_error_envelopes_surface_as_ipc_errors() -> anyhow::Result<()> {
        ...
        let raw = client
            .request(IpcEnvelope::request(
                names::session::START,
                Some(json!({ "tariffId": "x" })),
            ))
            .await?;
        assert!(raw.is_error());
        assert_eq!(
            raw.error.as_ref().map(|e| e.code),
            Some(ErrorCode::SessionAlreadyActive)
        );

        // … while the typed helpers convert it.
        let err = client
            .call_raw(names::session::START, None)
            .await
            .unwrap_err();
        assert!(
            matches!(err, WinUtilError::Ipc(ref e) if e.code == ErrorCode::SessionAlreadyActive),
            "{err}"
        );
